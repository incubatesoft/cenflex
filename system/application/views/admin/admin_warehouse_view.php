...
//alert(gr); 
    ...
    colNames:['id','Factory Code','Contact Name','Mobile','Factoty Phone','Street1','street2','City','State','username','password','Pin Code','Tin Number','Edit','Delete'],
		 ...
		 {name:'contactname',index:'contactname', width:100,editable:true},
		  {name:'mobile',index:'factorycode', width:80,editable:true},
		   {name:'factoryphone',index:'factorycode', width:100,editable:true},
		    {name:'street1',index:'factorycode', width:80,editable:true},
 			 {name:'street2',index:'factorycode', width:80,editable:true},
			  {name:'city',index:'factorycode', width:80,editable:true},
			   {name:'state',index:'factorycode', width:80,editable:true},
    		  {name:'username',index:'factorycode', width:80,editable:true},
			   {name:'password',index:'factorycode', width:80,editable:true},
...
$("#dialog_message").dialog({
		...
		buttons: { 
				"Ok": function() {
					...
					$.ajax({
					type: "POST",
					url: base_url+"admin/admin_modules/warehouse_delete/"+jQuery("#list5").getGridParam('selrow'),			 
					beforeSend: function()
					{
...
function  delete_sem(obj)
{

 		$(this).closest("td").unbind("click");
		// alert($(obj).attr("ship_no"));
		$("#delete_idval").val($(obj).attr("id"));
		var msg="Are you sure you want to delete Warehouse "+obj + "?";
 ...
 function  edit_ware(obj)
 {
 		//alert('ffff');
		//alert($(obj).attr("ship_no"));
		
		$("#add_admin_shipping").width(430).height(400);
		
		//$(this).closest("td").unbind("click");
 		
		$.blockUI({message: $("#add_admin_shipping")});
		
		$("#add_admin_shipping .modal_title").html("Edit Warehouse Details");
		
		  var query_str = "";
		  	$.ajax({
			type: "POST",
			url: base_url+"admin/admin_modules/warehouseedit/"+obj,
		 //	data: query_str,
			beforeSend: function(){
