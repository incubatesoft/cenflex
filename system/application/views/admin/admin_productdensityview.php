...
//alert(gr); 
    ...
    url:base_url+'admin/admin_itemdensitiesgrid',
    ...
    colNames:['id','Product Name','Variety','Density','Edit','Delete'],
    ...
    colModel:[
	    {name:'id',index:'id', width:80},
		 ...
		 {name:'itemname',index:'itemname', width:150,editable:true},
		  {name:'variety',index:'itemname', width:150,editable:true},
		
		 {name:'density',index:'density', width:110,editable:true},
    ...
    viewrecords: true,
    sortorder: "desc",
    caption:"Product Densities List",
	height:260,
...
//alert('hi');
	...
	$.ajax({
		type: "POST",
		 url: base_url+'admin/admin_modules/density_delete/'+obj,
		//data: $('#sendorderForm').serialize(),
		
		success: function(msg)
		{
		//alert(msg);	
		if(msg=="Product density Deleted Successfully.")
				{
				//success(1,msg);
 ...
 function  edit_productdensities(obj)
 {
 		//alert('ffff');
		//alert($(obj).attr("ship_no"));
		...
		$("#add_admin_shipping .modal_title").html("Edit Product Density");
		
		  var query_str = "";
		  	$.ajax({
			type: "POST",
			url: base_url+"admin/admin_modules/Editproductdensity/"+obj,
		 //	data: query_str,
			beforeSend: function(){
