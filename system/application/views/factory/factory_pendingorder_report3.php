...
$branchfinaltotalmtrs=0;
	...
	$parties=mysql_query("select distinct(orderby) from tbl_order_master where status!='Dispatched' and status!='New' and orderto='".$vars_updated."'");
	while($partiesrow=mysql_fetch_array($parties))
	{
		
		$str=mysql_query("select * from tbl_order_master where status!='Dispatched' and status!='New' and orderby='".$partiesrow['orderby']."'  and orderto='".$vars_updated."'ORDER BY id desc");
