...
$branchfinaltotalmtrs=0;
	...
	$parties=mysql_query("select distinct(orderby) from tbl_order_master where orderto='".$branchesrow['orderto']."' and status!='New'");
	while($partiesrow=mysql_fetch_array($parties))
	{
		
		$str=mysql_query("select * from tbl_order_master where  orderby='".$partiesrow['orderby']."'  and status!='New' and orderto='".$branchesrow['orderto']."' ORDER BY id desc");
