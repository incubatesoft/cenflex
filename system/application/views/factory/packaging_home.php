...
<thead>
<tr><th nowrap="nowrap">S No</th><th nowrap="nowrap">Date</th><th>Package ID</th><th>View</th></tr>
</thead><tbody>
<?php
...
{
$i++;
$res_date=mysql_query("select id,date,donum from tbl_packaginglist where pdfpath='".$row1['pdfpath']."'");
//echo "select *from tbl_orders where donum='".$row1['donum']."'";

$s1=mysql_fetch_array($res_date);


$pkyr=substr($s1['date'], -4);
echo "<tr><td id='confirm-dialog'>".$i."</td><td id='confirm-dialog' style='padding-left:10px;'>".$s1['date']."</td><td>".$s1['id']."/".$pkyr."</td><td id='confirm-dialog'><a href='".base_url().$row1['pdfpath']."' target=_new style=color:black;text-decoration:underline>Details</a></td></tr>";
...
<tfoot>
<tr><th></th><th></th><th></th><th></th></tr></tfoot>
