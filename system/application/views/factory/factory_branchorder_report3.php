...
<!-- start of table <table cellpadding="3" border=0 class="display KeyTable"  width=100%  style="border:solid 1px #960">  -->
...
$branchfinaltotalmtrs=0;
	...
	$parties=mysql_query("select distinct(orderby) from tbl_order_master where orderto='".$vars_updated."' and status!='New'");
	while($partiesrow=mysql_fetch_array($parties))
	{
		
		$str=mysql_query("select * from tbl_order_master where  orderby='".$partiesrow['orderby']."'  and status!='New' and orderto='".$vars_updated."' ORDER BY id desc");
