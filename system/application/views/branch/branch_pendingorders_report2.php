...
<td align="right" style="padding-right:25px;"><a href=" <?php echo base_url(); ?>index.php/branch/branch_orders/pendingordersprintview/<?php echo $vars?>" target="_blank"><img src="<?php echo base_url(); ?>images/print.gif"  style="border:none"/></a></td></tr>
...
{ //alert('hi');
	...
	$.ajax({
			type: "POST",
			url: base_url+"branch/branch_orders/pendingreports_paginate/"+obj,
		 //	data: query_str,
			beforeSend: function(){
...
$parties=mysql_query("select distinct(orderby) from tbl_order_master where orderto='".$row_branch['branchname']."'  and status!='Dispatched'");
	while($partiesrow=mysql_fetch_array($parties))
	{
    $co = mysql_query("select * from tbl_order_master where  orderby='".$partiesrow['orderby']."'  and orderto='".$row_branch['branchname']."' and status!='Dispatched' ORDER BY id desc ");
...
$branchfinaltotalmtrs=0;
	...
	while($partiesrow=mysql_fetch_array($parties))
	{
		...
		$str=mysql_query("select * from tbl_order_master where  orderby='".$partiesrow['orderby']."'  and orderto='".$row_branch['branchname']."' and status!='Dispatched' ORDER BY id desc  LIMIT 0, 10");
