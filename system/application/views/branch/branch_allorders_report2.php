...
<tr><td class="heading" style="padding-top:30px;">All Orders Report</td>
<td align="right" style="padding-right:25px;"><a href=" <?php echo base_url(); ?>index.php/branch/branch_orders/allordersprintview/<?php echo $vars?>" target="_blank"><img src="<?php echo base_url(); ?>images/print.gif"  style="border:none"/></a></td></tr>



<?php $res_branch=mysql_query("select *from tbl_branches where username='".$this->db_session->userdata('u_name')."'");
...
{ //alert('hi');
	...
	$.ajax({
			type: "POST",
			url: base_url+"branch/branch_orders/allreports_paginate/"+obj,
		 //	data: query_str,
			beforeSend: function(){
...
$parties=mysql_query("select distinct(orderby) from tbl_order_master where orderto='".$row_branch['branchname']."'");
	while($partiesrow=mysql_fetch_array($parties))
	{
    $co = mysql_query("select * from tbl_order_master where  orderby='".$partiesrow['orderby']."'  and orderto='".$row_branch['branchname']."' ORDER BY id desc ");
	$count = mysql_num_rows($co);
	$final_count = round($count/10);
...
$branchfinaltotalmtrs=0;
	...
	$str=mysql_query("select * from tbl_order_master where  orderby='".$partiesrow['orderby']."'  and orderto='".$row_branch['branchname']."' ORDER BY id desc LIMIT 0, 10");
		//echo "select * from tbl_order_master where orderby='".$partiesrow['orderby']."'  and orderto='".$row_branch['branchname']."' ORDER BY id desc";
?>
...
//	echo "<tr class=odd><td colspan=3 style='color:#960;'><b>Total Bundles/Meters </td><td style='color:#960;'><b>".$allfinaltotalbundles."/".$allfinaltotalmtrs."</b></td><td style='color:#960;'><b>".$alltotaldispatched."/".$alltotaldispatchedmtrs."</b></td><td style='color:#960;'><b>".$alltotalpendingbundles."/".$alltotalpendingmtrs."</b></td><td></td></tr>";
?>
