...
{	
	...
	* 
	* @uses libraries (encrypt, db_session), helpers (form, url, FreakAuth), modules (Usermodel)
	* */
     ...
     * 3) $this->freakauth_light->check('admin', true)  //this restricts acess to 'admin' ONLY
     * 
     ...
     *
     * if FAL_flash == true in config file, displays a flash message
     *      and redirect to the referer page (or root if none)
     *
     * else displays the FAL_denied_page (see config file)
     *
     * @param $role : the role of the one we are denying the access
     */
    function denyAccess($role)
    {
        $this->CI->lang->load('freakauth');
        if ($this->CI->config->item('FAL_flash'))
        {
            ...
            if ($role == '')
            {
                ...
                // maybe if we can log in, we'll have some more permissions
...
+                       redirect($this->CI->config->item('FAL_denied_from_ext_location'));
                    ...
                    {
                        // if we come from this website, just go to this page back
    ...
    function belongsToGroup($_group=null, $_only=null)
    {
        if ($this->CI->db_session AND $this->CI->config->item('FAL'))
        {
            ...
            if ($_username != false AND $_who_is != false)
            {
	            ...
	            $_group = array();
	            //eliminate possible whitespaces at the beginning and end of groups names
    ...
    {            
            ...
            if (($username != false) && ($password != false))
            {
                ...
                if ($query->num_rows() == 1)
                {
                    ...
                    if ($row->{'banned'} == 0)
                    {
                        ...
                        //set FLASH MESSAGE
                        $this->CI->db_session->set_flashdata('flashMessage', $this->CI->lang->line('FAL_login_message'), 1);
        ...
        //set FLASH MESSAGE
        $this->CI->db_session->set_flashdata('flashMessage', $message, 1);
    ...
    function register()
    {
        ...
        //set FLASH MESSAGE
        $this->CI->db_session->set_flashdata('flashMessage', $this->CI->lang->line('FAL_invalid_register_message'), 1);
        return false;
    }
    ...
    // --------------------------------------------------------------------
    /**
     * Convenient way to generate html code displaying the captcha
     * using 'img' tag in a 'span class="note"' tag
     *
     * @return html code to show the captcha
     */
    function show_captcha() {
	//echo $this->CI->config->item('FAL_security_code_image_path');
	  $this->_generateCaptcha('preview');
	
	//$this->CI->db_session->unset_userdata('FreakAuth_security_code');
	
        $img_html = '<span class="note">';
        $img_html.= '<img src="'.base_url().$this->CI->config->item('FAL_security_code_image_path').$this->CI->config->item('FAL_security_code_image').'" alt="" />';
        $img_html.= '</span>';
        return $img_html;
    }
     ...
     * remain in the root/tmp folder before it will be deleted.  The default is 10 minutes. Change the value of $expiration
    ...
    {
       // echo $now; exit;
	  // echo BASEPATH.'fonts/Jester.ttf';
	   $now .= time();
	   
	  
            $securityCode = $this->_generateRandomString(5, 6);
			
			 $_SESSION['scode'] =  $securityCode;
			//$image = 'security-'.$this->_generateRandomString(16, 32).'.jpg';
			$image = $now.'.jpg';
            $this->CI->config->set_item('FAL_security_code_image', $image);
            
            $config['image_library'] = 'GD2';
            $config['source_image'] = 'public/images/captcha/base_image.jpg';
            $config['new_image'] =  'tmp/'.$image;
            $config['wm_text'] = $securityCode;
            $config['wm_type'] = 'text';
            $config['wm_font_path'] =  BASEPATH.'fonts/Jester.ttf';
            $config['wm_font_size'] =  20;
            $config['wm_font_color'] = '5666666';
          ...
          //  $this->CI->db_session->set_userdata('FreakAuth_security_code', $securityCode);			
  	...
  	{	
  		...
  		{	
	  		...
	  		else 
		    {
		       ...
		       //set FLASH MESSAGE
		       // $this->CI->db_session->set_flashdata('flashMessage', 'No profile found for this user', 1);
