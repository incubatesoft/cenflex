<?php
class Admin_itemdensitiesgrid extends Controller 
{
...
if($searchOn=='true') 
{
	...
	if( $fld=='id' || $fld =='itemname' || $fld =='variety' || $fid=='density') 
	{
...
$result = mysql_query("SELECT COUNT(*) AS count FROM item_densities where id>0".$wh);
...
$SQL = "SELECT  id,itemname,variety,density from item_densities  where id>0 ".$wh. " ORDER BY ".$sidx." ". $sord." LIMIT ".$start." , ".$limit;
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC))
 {
    ...
    $responce->rows[$i]['cell']=array($row['id'],$row['itemname'],$row['variety'],$row['density'],"<button id='edit_idval'  class='ui-button ui-state-default ui-corner-all' style='height:18px; width:70px;' value='$row[id]' onClick=edit_productdensities('$row[id]')>Edit
</button>","<button id='delete_idval'  class='ui-button ui-state-default ui-corner-all' style='height:18px; width:70px;' value='$row[id]' onClick=delete_density('$row[id]')>Delete
</button>");
