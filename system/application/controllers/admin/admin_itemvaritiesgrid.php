<?php
class Admin_itemvaritiesgrid extends Controller 
{
...
if($searchOn=='true') 
{
	...
	if( $fld=='id' || $fld =='itemname' || $fld =='variety' ) 
	{
...
$result = mysql_query("SELECT COUNT(*) AS count FROM tbl_item_varieties where id>0".$wh);
...
$SQL = "SELECT  id,itemname,variety from tbl_item_varieties  where id>0 ".$wh. " ORDER BY ".$sidx." ". $sord." LIMIT ".$start." , ".$limit;
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC))
 {
    ...
    $responce->rows[$i]['cell']=array($row['id'],$row['itemname'],$row['variety'],"<button id='edit_idval'  class='ui-button ui-state-default ui-corner-all' style='height:18px; width:70px;' value='$row[id]' onClick=edit_productvariets('$row[id]')>Edit
</button>","<button id='delete_idval'  class='ui-button ui-state-default ui-corner-all' style='height:18px; width:70px;' value='$row[id]' onClick=delete_ivaiety('$row[id]')>Delete
</button>");
    $i++;
}
