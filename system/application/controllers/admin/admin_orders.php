...
echo "<script>window.location.href='".base_url()."index.php/login/loginsuggestion'</script>";
		...
		function loadingordersprintview($val) 
	{
		...
		{
		$distributors=$distributors.$row_dis['dname'].",";
		}
		echo $distributors;
		}
		else
	...
	//echo 'hi';
		...
		{
		$branches=$branches.$row_br['branchname'].",";
		}
		echo $branches;
		}
		else
		...
		{
		//echo $val;
		$str=str_replace("%20"," ",$val);
		//echo "select * from tbl_distributors where dname='".$str."' ";
		$cities=mysql_query("select * from tbl_distributors where dname='".$str."' ");
		if(mysql_num_rows($cities)>0)
		{$citynames=mysql_fetch_array($cities);
		echo $citynames['city'];
		} else {		
		$cities=mysql_query("select * from tbl_branches where branchname='".$str."' ");
		$citynames=mysql_fetch_array($cities);
		echo $citynames['city'];
		}
