<?php
class Admin_itemgrid extends Controller 
{
...
if($searchOn=='true') 
{
	...
	if( $fld=='id' || $fld =='itemcode' || $fld =='itemname' || $fld =='unit') 
	{
...
$result = mysql_query("SELECT COUNT(*) AS count FROM tbl_itemmaster where id>0".$wh);
...
$SQL = "SELECT  id,itemcode,itemname,unit from tbl_itemmaster where id>0 ".$wh. " ORDER BY ".$sidx." ". $sord." LIMIT ".$start." , ".$limit;
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC))
 {
    ...
    $responce->rows[$i]['cell']=array($row['id'],$row['itemcode'],$row['itemname'],$row['unit'],"<button id='edit_idval'  class='ui-button ui-state-default ui-corner-all' style='height:18px; width:70px;' value='$row[id]' onClick=edit_product('$row[id]')>Edit
</button>","<button id='delete_idval'  class='ui-button ui-state-default ui-corner-all' style='height:18px; width:70px;' value='$row[id]' onClick=delete_product('$row[itemname]')>Delete
</button>");
    $i++;
}
