<?php
class Factory_usersgrid extends Controller 
{
...
if($searchOn=='true') 
{
	...
	if( $fld=='id' || $fld =='name' || $fld =='street1' || $fld =='street2' || $fld =='mobile' || $fld =='city'||  $fld =='state' || $fld =='pincode')
	{
...
$result = mysql_query("SELECT COUNT(*) AS count FROM tbl_factoryusers where id>0 ".$wh);
...
$SQL = "SELECT  * from tbl_factoryusers where id>0 ".$wh. " ORDER BY ".$sidx." ". $sord." LIMIT ".$start." , ".$limit;
...
,$row['name'],$row['mobile'],$row['street1'],$row['street2'],$row['city'],$row['state'],$row['username'],$row['password'],$row['pincode'], "<button id='edit_idval'  class='ui-button ui-state-default ui-corner-all' style='height:18px; width:70px;' value='$row[id]' onClick=deleteuser('$row[id]')>Delete</button>");
    $i++;
}
