$(document).ready(function(){
	...
	newWidths = false;
		...
		$(".save_disk").live("click", function(){
				...
				var queryString="english="+english+"&chinese="+chinese+"&romanized_chinese="+romanized_chinese;
				 $.ajax({
				   type: "POST",
				   url: base_url+'chinese/update_china_eng',
				   data: queryString,
	...
	dataReady();
	//Sorting Functions
	$("#ac_tablehead .row div").live("click", function(){
		sortTable($("#ac_tablehead .row div").index(this));
		
	})
	var aAsc = [];
	function sortTable(nr) {
	aAsc[nr] = aAsc[nr]=="asc"?"desc":"asc";
	$("#ac_table .row").tsort("td div:eq("+nr+")[title]",{order:aAsc[nr]});
	$("#ac_table .row:odd").removeClass("altBlu");
	//alert("Done")
...
function dataReady(){
	...
	function delete_china(id)
	{
		var queryString="id="+id;
				 $.ajax({
				   type: "POST",
				   url: base_url+'chinese/delete_china_eng',
				   data: queryString,
...
function delay_search(){
		if($(".search").val()!=""){
			...
			st = setTimeout("searchme(searchterm)", 10);
			var st1 = "active";
		}else{
			$(".clear_search").hide();
			$("#ac_table .row").show();
...
function searchme(searchterm){
	// $("#overlay").show().fadeTo(50, 0.3);
	$("#ac_table .row").hide();
	$("#ac_table .row > td div:contains("+searchterm.toLowerCase()+")").closest(".row").show();
	var oto = setTimeout('$("#overlay").fadeOut()', 200);
}
function editInplace(){
	alternatingRows();
		$('#ac_table .eng:not(.lastrow .eng)').editable(base_url+'chinese/edit_china_eng', {
				 ...
				 $('#ac_table .chin:not(.lastrow .chin)').editable(base_url+'chinese/edit_china', {
					 indicator :'<img src="'+base_img+'/ajax loader2.gif'+'"/>',
					 height: 'none',
					 width: 'none',
					 cssclass : 'editing',
				  ...
				  $('#ac_table .rom:not(.lastrow .rom)').editable(base_url+'chinese/edit_china_rom', {
					 indicator :'<img src="'+base_img+'/ajax loader2.gif'+'"/>',
					 height: 'none',
					 width: 'none',
					 onblur:'submit',
...
function calculateWidths(){
		var w = 0;
		jQuery.each($(".row> div:visible"), function(){
			w = w + parseInt($(this).width());
		});
		var colName = $(".row > div").length
		$(".row").css("min-width", w+(colName*8));
		$("#ac_tablehead .row").css("min-width", w+(colName*8)+20);
}
