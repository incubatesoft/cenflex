...
$.fn.jqGrid = function( p ) {
	...
	grid_pager: 1,
	surya_click:"no",
	functionname:"listview/showInnerGrid",
	height: 150,
	page: 1,
	rowNum: 20,
	records: 0,
	...
	$.fn.addRowData = function(rowid,data,pos,src) {
		...
		if(data) {
			this.each(function() {
				...
				if(t.p.subGrid ) { try {$(t).addSubGrid(t.grid.bDiv,row,gi,ts.p.functionname);} catch(e){} si=1;}
	...
	return this.each( function() {
		...
		var addCell2 = function(t,row,cell,pos) {
			var td;
			td = document.createElement("td");
			$(td).addClass('edit_hover');			 			
			$(td).html(cell);			 
			row.appendChild(td);
			formatCol($(td,t), pos);
		};
		...
		var addXmlData = function addXmlData (xml,t) {
			...
			$(ts.p.xmlReader.root+" "+ts.p.xmlReader.row,xml).each( function( j ) {
				...
				if (ts.p.subGrid) {
				//	alert();
					try {$(ts).addSubGrid(t,row,gi,this,ts.p.functionname);} catch (e){}
					si= 1;
				}
				var v;
				if(ts.p.xmlReader.repeatitems===true){
					$(ts.p.xmlReader.cell,this).each( function (i) {
						...
						if(ts.p.colModel[i+gi+si].editable)
						addCell2(t,row,v || '&#160;',i+gi+si);						 
						else
		...
		var addJSONData = function(data,t) {
			...
			for (var i=0;i<drows.length;i++) {
				...
				if (ts.p.subGrid) {
					try { $(ts).addSubGrid(t,row,gi,drows[i],ts.p.functionname);} catch (e){}
					si= 1;
				}
	...
	var updatepager = function() {
			if(ts.p.pager) {
			    ...
			    //alert(ts.p.surya_click)
					   
			/*	$(ts.p.pager).append("<div class='prev_nextWrap'><span id='surya_span' class='pageinfo'>Showing "+1+" - "+(1*ts.p.rowNum)+" of </span><span id='sp_2'  class='pageinfo'></span>"+pgl+pgr+"</div>");*/
			
			 var cp_new = IntNum(ts.p.page);
			// alert(ts.p.records)
			var first_val = "";
			var second_val = "";
					...
					if(ts.p.records!=0 )
					{
					  ...
					  if(ts.p.surya_click=="no" )
					  { 
						first_val =  ts.p.page; 	
					  	second_val = ts.p.page*ts.p.rowNum;
					  }
					  else 
					  {
							 first_val = ((cp_new-1)*ts.p.rowNum)+1;
						     second_val = (cp_new)*ts.p.rowNum;  
					  }
					  
					  if(ts.p.records<second_val)
					  {
					 	cur_page_val = 	ts.p.records;  
					  }else
					  {
						cur_page_val = 	 second_val; 
					  }
					  initial_val = "Showing "+first_val+" - "+cur_page_val+" of ";
					} 
					
					$('#surya_span',ts.p.pager).html(initial_val);
				...
				if(ts.p.pgbuttons===true) {
					...
					/***start: mousedown & up: events added by RajuBogi***/	
					//Prev button
					(!eval($("#prev").attr('disabled')))?
						$("#prev").bind("mousedown", function(e){
							$(this).attr("src", ts.p.imgpath + "dn-" + ts.p.previmg);
						}).bind("mouseup", function(e){
							$(this).attr("src", ts.p.imgpath + "off-" + ts.p.previmg);
						}) : $("#prev").unbind("mousedown").unbind("mouseup");
					//next button				
					(!eval($("#next").attr('disabled')))?
						$("#next").bind("mousedown", function(e){
							$(this).attr("src", ts.p.imgpath + "dn-" + ts.p.nextimg);
						}).bind("mouseup", function(e){
							$(this).attr("src", ts.p.imgpath + "off-" + ts.p.nextimg);
						}) : $("#next").unbind("mousedown").unbind("mouseup");					
					/***end: mousedown & up: events added by RajuBogi***/
				}
			}
	...
	var populate = function () {
		...
		/// this is the customaized code for pagination done by surya code starts
		
	    var setPager = function (){
		 
			
			var inpt = "<img src='"+ts.p.imgpath+"spacer.gif'";
			 
			var pgl="", pgr="";
			if(ts.p.pgbuttons===true) {
				pgl =  inpt+" class='pgbuttons prev' id='prev' width='19' height='17' />";
				pgr = inpt+" class='pgbuttons next' id='next' width='20' height='17' />&nbsp;";
			}
			 
				var selectBox="<SELECT class='selbox'>";
				for(var i=0;i<ts.p.rowList.length;i++){
					selectBox +="<OPTION value="+ts.p.rowList[i]+((ts.p.rowNum == ts.p.rowList[i])?' selected':'')+">"+ts.p.rowList[i];
				}
				selectBox +="</SELECT>";
			 
			    
				$(ts.p.pager).append("<div class='prev_nextWrap'><span id='surya_span' class='pageinfo'></span><span id='sp_2'  class='pageinfo'></span>"+pgl+pgr+"</div>");
		...
		/// this is the customaized code for pagination done by surya code ends
		
		
		var setPager2 = function (){
